...
window.addEventListener("DOMContentLoaded", function () {
  ...
  var moduleSlider = new _modules_slider_slider_main__WEBPACK_IMPORTED_MODULE_0__["default"]({
    container: ".moduleapp",
    btns: ".next "
  }); // Slider на другій сторінці

  moduleSlider.render();
  var videoPlayer = new _modules_videoPlayer__WEBPACK_IMPORTED_MODULE_2__["default"]('.showup .play', '.overlay');
  videoPlayer.init();
  new _modules_videoPlayer__WEBPACK_IMPORTED_MODULE_2__["default"]('.module__video-item .play', '.overlay').init(); // videoplayer на modules.html
...
/*#__PURE__*/
function () {
  function DownloadFile(triggersSelector) {
...
function () {
  ...
  _createClass(Form, [{
    ...
    value: function checkMailInputs() {
      var mailInputs;
      return regeneratorRuntime.async(function checkMailInputs$(_context) {
        while (1) {
              ...
              mailInputs.forEach(function (input) {
                ...
                input.addEventListener("blur", function (e) {
                  ...
                  if (input.value.match(/[^a-z 0-9 @ \.]/ig)) {
                    ...
                    input.value = "";
                    var test = input.getAttribute("placeholder");
                    var res = "Only English letters !!!";
    ...
    value: function initMask() {
      var setCursorPosition = function setCursorPosition(pos, elem) {
        ...
        if (elem.setSelectionRange) {
          ...
          // for Internet Explorel and old browsers
          var range = elem.createTextRange(); // internet
          //#region 
      ...
      function createMask(event) {
        ...
        this.value = matrix.replace(/./g, function (a) {
          // Це |value| буде відображатись на сторінці
...
function () {
  //modules.html
  function ShowInfo(triggersSelector) {
...
function (_Slider) {
  ...
  _createClass(MainSlider, [{
    ...
    value: function bindTriggers() {
      ...
      document.querySelectorAll('.next .nextmodule').forEach(function (btn) {
        btn.addEventListener('click', function (e) {
          e.preventDefault();
          e.stopPropagation(); // read in the Internet
    ...
    value: function render() {
      if (this.container) {
        /* для роботи в конкретній сторінці сайту */
        try {
  ...
  return MainSlider;
}(_slider__WEBPACK_IMPORTED_MODULE_9__["default"] // Наслідування
);
...
function (_Slider) {
  ...
  function MiniSlider(container, previous, next, activeClass, animate, autoplay) {
    _classCallCheck(this, MiniSlider);

    return _possibleConstructorReturn(this, _getPrototypeOf(MiniSlider).call(this, container, previous, next, activeClass, animate, autoplay));
  }
  ...
  _createClass(MiniSlider, [{
    ...
    value: function decorizeSlides() {
      ...
      if (!this.slides[0].closest('button')) {
        /* Якщо актив. слайд не кнопка то додаєм activeClass */
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (btn, i) {
        try {
          var blockedElem = btn.closest('.module__video-item').nextElementSibling; // Отримуєм першого батька а потім його сусіда
            ...
            if (document.querySelector('iframe#frame')) //fix bug || Коли другий раз відкpиваєм відео,то воно не запускається
              {
                ...
                if (_this.path !== btn.getAttribute('data-url')) {
                  // Якщо юзер клікнув на іншу кнопку
                  _this.path = btn.getAttribute('data-url'); // перезаписуєм шлях до відео
              ...
              _this.path = btn.getAttribute('data-url'); // Шлях до відео
    ...
    value: function onPlayerStateChange(state) {
      try {
        ...
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; // Отримуєм першого батька а потім його сусіда

        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); // full copy
